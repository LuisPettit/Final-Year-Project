...
def json_graph(account):#, x):
    ...
    for i in range(len(users)):
        ...
        #friends_list = api.friends(screen_name = users[i]._json['screen_name'], count = 2)
...
def graph():
	...
	with open('static/fearghus_9_6_4.json') as f:
		ferg = json.load(f)
	f.close()
	...
	for i in range(len(ferg['nodes'])):
    
		tmp = []
    
		tmp.append(ferg['nodes'][i]['screen_name'])
		tmp.append(ferg['nodes'][i]['label'])
    
       
		if (ferg['nodes'][i]['label'] == 'Bot') or (ferg['nodes'][i]['label'] == '{bot}'):
...
def results():
    ...
    data = [['Classification', y], ['Classification Score', proba], ['Account Score', account_score], ['Sentiment Score', sentiment_score], ['Activity Score', activity_score], ['Interactiveness Score', interactiveness_score], ['Tweet Source Score', tweet_source_score]]
